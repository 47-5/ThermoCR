...
def run_temperature_simulation(config_file_path, *, verbose=False):
    ...
    for simulator_index, (T, t_start, t_end) in enumerate(zip(T_list, t_start_list, t_end_list)):
        ...
        simulator = ChemicalKineticsSimulator(
...
def load_config(config_file):
    """Load a YAML simulation config file."""
...
def parser_T_program(T_program_config):
    ...
    t_start_list = []
    t_end_list = []
    for T_segment in T_program_config:
        segment_type = T_segment["type"]
        if segment_type == "constant":
            ...
            segments = int(T_segment["segments"])
            if segments < 1:
                raise ValueError("linear temperature-program segments must be >= 1")

            T_s = list(np.linspace(start=T_segment["T_start"], stop=T_segment["T_end"], num=segments))
            t_grid = np.linspace(start=T_segment["t_start"], stop=T_segment["t_end"], num=segments + 1)
            t_start_s = list(t_grid[:-1])
            t_end_s = list(t_grid[1:])
            T_list += T_s
            t_start_list += t_start_s
            t_end_list += t_end_s
        else:
            raise ValueError(f"Unsupported temperature-program segment type: {segment_type}")
...
def plot_t_y_T(all_t, all_y, all_T, add_T=True, save_path="t_y_T.png"):
    ...
    for i, species in enumerate(all_y):
        plt.plot(all_t, species, label=f"S{i + 1}")
    plt.xlabel("Time / s")
