"""Reaction-system simulation helpers."""
...
from ThermoCR.io import read_imaginary_vib


class ChemicalKineticsSimulator:
    def __init__(self, config_file, override_temperature=None,
                 override_initial_concentrations=None, override_time_span=None):
        self.config_file = Path(config_file)
        self.config_dir = self.config_file.resolve().parent
        self.config = self.load_config(self.config_file)

        self.system_config = self.config['system']
        # 使用覆盖的温度（如果提供）
        self.T = override_temperature if override_temperature is not None else self.system_config['T']
        self.P = self.system_config['P']
        self.R = 8.314462618  # J/(mol·K)

        # 使用覆盖的初始浓度（如果提供）
        if override_initial_concentrations is not None:
            self.config['system']['initial_concentrations'] = override_initial_concentrations

        # 使用覆盖的时间范围（如果提供）
        if override_time_span is not None:
            self.config['system']['time_span'] = {
                'start': override_time_span[0],
                'end': override_time_span[1]
            }
    ...
    def _resolve_on_the_fly_paths(self, params):
        if params is None:
            return {}
        resolved = dict(params)
        for key in ('atom_coord_path', 'vib_path', 'ee_path'):
            resolved[key] = self._resolve_config_path(resolved.get(key))
        return resolved
    ...
    def thermo_on_the_fly(self, on_the_fly_params):
        params = self._resolve_on_the_fly_paths(on_the_fly_params)
        thermo_result = qm_thermo(
            atom_coord_path=params.get('atom_coord_path', None),
            atom_numbers=params.get('atom_numbers', None),
            coords=params.get('coords', None),
            vib_path=params.get('vib_path', None),
            vibfreqs=params.get('vibfreqs', None),
            ee_path=params.get('ee_path', None),
    ...
    def calculate_TST_rate_constants(self, reaction_info, TST_params):
        ...
        ts_params = TST_params.get('TS_on_the_fly_params')
        ...
        if TST_params.get('tunnelling_effect'):
            resolved_ts_params = self._resolve_on_the_fly_paths(ts_params)
            imaginary_freq = read_imaginary_vib(filepath=resolved_ts_params['vib_path'])
    ...
    def plot_results(self, save_path=None):
        ...
        all_t, all_y = self.solution['t'], self.solution['y']
    ...
    def export_result(self, save_path=None):
        if not hasattr(self, 'solution'):
            print("请先运行模拟!")
            return
        # print(self.solution)
        ...
        results_dict = {'t': t}
        for index, species in enumerate(y):
            results_dict[f'species{index+1}'] = species
...
#     simulator = ChemicalKineticsSimulator('reaction_system_small.yaml')
