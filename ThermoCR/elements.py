"""Element metadata used by ThermoCR."""
...
def element_data_by_atomic_number(atomic_number):
    """Return the raw element data row for an atomic number."""
    return atom_data[int(atomic_number)]


def atomic_symbol(atomic_number):
    """Return the element symbol for an atomic number."""
    return element_data_by_atomic_number(atomic_number)[1]


def atomic_mass(value):
    """Return atomic mass for an atomic number or element symbol."""
    if isinstance(value, int):
        return element_data_by_atomic_number(value)[3]
    return element_mass(value)


def atomic_masses(atomic_numbers):
    """Return atomic masses for an iterable of atomic numbers."""
    return [atomic_mass(int(atomic_number)) for atomic_number in atomic_numbers]


def element_mass(symbol):
    for element in atom_data:
        if element[1] == symbol.capitalize():
            return element[3]
